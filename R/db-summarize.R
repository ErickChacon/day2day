...
db_summarize <- function(path_data, filename = "summary-databases.txt"){
  ...
  write("------------------------------------------------", filename)
  write("Summary of databases inside the provided folder.", filename, app = T)
  write("------------------------------------------------", filename, app = T)
  for(i in 1:length(databases)){
    write(paste0("\n", i, ") RData file: ", databases[i]), filename, app = T)
    ...
    for(j in 1:length(obj_names)){
      ...
      write(paste0(i, ".", j, ") Object: ", obj_names[j]), filename, app = T)
      write(paste(var_names, collapse = " "), filename, app = T)
      }
    }
  }
}
